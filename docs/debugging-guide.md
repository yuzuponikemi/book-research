> [!WARNING]
> **⚠️ OUTDATED — このドキュメントは旧アーキテクチャ（`main.py` / LangGraph StateGraph）に基づいており、コマンド例や内部構造が現在のマイクロサービス構成と異なります。**
> 新しいデバッグ情報は [`docs/usage-guide-v2.md`](usage-guide-v2.md) のトラブルシューティングセクションを参照してください。
