> [!WARNING]
> **⚠️ OUTDATED — プロンプトの実装場所が変わりました（`src/reader/`, `src/director/` → `cogito/services/*/`）。基本のプロンプト内容は継承されていますが、ファイルパスや関数名が異なります。**
> 現在のプロンプト実装は `cogito/services/` 内の各サービスファイルを参照してください。
